...
import PIXLTicker from './components/PIXLTicker';
import PixlLabsFooter from './components/PixlLabsFooter';
import Signature from './components/Signature';

function App() {
  const [pixlPetId, setPixlPetId] = useState('');
  const [pixlPetData, setPixlPetData] = useState({ isHatched: false, imgUrl: '', message: '' })
  ...
  const handleSubmit = async () => {
    try {
      ...
      const isHatched = data.attributes.some(attr => attr.trait_type === "Egg" && attr.value === "Hatched");
      ...
      const element = data.attributes.find(attr => attr.trait_type === "Element").value;
      let animationUrl = "";
  ...
  return (
    <div className="App">
      ...
      {pixlPetData.imageUrl && (
        <div>
          <div className='pixl-text' dangerouslySetInnerHTML={{ __html: pixlPetData.message }} />
          <img src={pixlPetData.imageUrl} alt="Pixl Pet" className='pixl-image'/>
        </div>
      )}
            </main>
      <div>
